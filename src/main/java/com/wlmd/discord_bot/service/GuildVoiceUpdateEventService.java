...
public class GuildVoiceUpdateEventService {
	...
	public void createSession(GuildVoiceUpdateEvent event) {
		...
		MemberModel user = memberRepository.findByGuild_GuildIdAndDiscordUserId(event.getGuild().getIdLong(), event.getMember().getIdLong());
		MemberActivityModel activity = memberActivityRepository.findByGuildIdAndDiscordUserId(event.getGuild().getIdLong() ,user.getDiscordUserId());
